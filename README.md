...
| Aula WIN 60 HE MAX | native read-only `5C/12/23/2B` polling | точный `1CA2:1902`, `FFA0:0001`, 65-байтный HID envelope, прошивка `App V1.1.6 / Feb 4 2026` и полный 17-транзакционный proof; поддержка firmware-proven, физическая клавиатура ещё не проверена |
...
4. Aula WIN60HE выполняет полный read-only proof на эксклюзивном интерфейсе;
5. SparkLink и Sayo выполняют собственные capability-проверки внутри общего `Backend_Init`;
6. UAP запускается только после формирования списка точных подтверждённых native HID interface paths.

Каждый native-backend:

- проверяет свой HID fingerprint;
- выполняет только известный capability probe;
- заявляет точный SetupAPI interface path только после валидного ответа;
- публикует общий список подтверждённых native-интерфейсов дочернему UAP до его HID enumeration.

Патч Soup применяет исключение **до `CreateFileW`**, поэтому UAP не успевает открыть endpoint, уже подтверждённый native-протоколом. Первый доказавший протокол получает только этот точный interface path; соседние интерфейсы с тем же `VID:PID` остаются независимыми.

Новое устройство, подключённое после запуска, может потребовать перезапуска HallJoy для безопасного арбитража до запуска UAP. Переподключение уже классифицированного точного интерфейса внутри текущего сеанса поддерживается native-worker.
...
Она отображает MAD68 A0, Hex80 `0x96`, Addressed `09/94/02`, Aula WIN60HE, SparkLink, Sayo и UAP/Wooting. Синие аналоговые полосы и зелёные цифровые точки превью сохранены. Цифровые события используются только интерфейсом и не участвуют в аналоговом управлении ViGEm.
...
- Aula WIN60HE проверена по прошивке, независимому oracle и автотестам, но без физической клавиатуры: реальный USB timing, hotplug и другие версии firmware пока не заявляются.
