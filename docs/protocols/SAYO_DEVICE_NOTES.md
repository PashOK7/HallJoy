...
- Maps the physical button index to the user's configured keyboard HID usage by
  matching a normal keyboard HID report only when exactly one still-held physical
  edge is inside the 80 ms association window. Simultaneous candidates are left
  unmapped rather than cross-learning a letter; a later isolated press retries
  automatically.
...
- The backend aggregates one O3C session; simultaneous separate O3C devices
  intentionally fail closed for automatic letter identity rather than sharing a
  global pending key.
