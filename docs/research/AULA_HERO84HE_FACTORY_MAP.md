...
`AULA_2829`; it contains 84 physical keys.  `position` is the two-byte ID to
...
The gaps are intentional: physical position IDs are not a contiguous 1..84
sequence.  The direct `94 02` path accepts the mapped physical IDs, not HID
usage values.

2026-09-19 correction: the earlier 83-key extraction omitted position 53. Its JavaScript label uses backticks, which the new static extractor handles explicitly. The pinned default array contains 84 keys. The native poll list and dedicated layout now include this position.
