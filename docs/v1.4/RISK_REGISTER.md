...
|---|---|---|---|---|---|
| `HJ-AUD-P1-001` | P1 | `TerminateThread` используется как обычный механизм shutdown | `S04/S05/S08` | Partial | realtime, logger and overlay removed and verified in V14-06A-C; SparkLink and Sayo remain |
...
S01 добавил общий lifecycle-контракт, generation-aware state machine и fault-injection seam. V14-05 подключил этот контракт к production registry и backend callbacks, поэтому `HJ-AUD-P1-003` и `HJ-AUD-P2-020` закрыты проверенными локальными gates. Риск `HJ-AUD-P1-001` остаётся `Partial`: V14-06A-C удалили принудительное завершение realtime, logger и overlay worker'ов, но SparkLink и Sayo ещё требуют миграции.
...
Пользовательский Windows/MSVC + реальный SparkLink gate S02B.2 пройден: analog input, ViGEm, режимы, reconnect и shutdown работают без регрессий. `TerminateThread` SparkLink остаётся отдельным риском S08.
