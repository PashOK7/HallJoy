...
HWND BindPanel_Create(HWND parent, HINSTANCE hInst)
{
    ...
    g_cbAction = CreateWindowW(WC_COMBOBOXW, L"",
        ...
        x, y + S(parent, 26), w, S(parent, 300), parent, reinterpret_cast<HMENU>(static_cast<INT_PTR>(ID_ACTION)), hInst, nullptr);
    ...
    g_btnBind = CreateWindowW(L"BUTTON", L"Bind",
        ...
        parent, reinterpret_cast<HMENU>(static_cast<INT_PTR>(ID_BIND)), hInst, nullptr);

    g_btnClear = CreateWindowW(L"BUTTON", L"Clear HID",
        WS_CHILD | WS_VISIBLE, x + S(parent, 86), y + S(parent, 58), S(parent, 100), S(parent, 26),
        parent, reinterpret_cast<HMENU>(static_cast<INT_PTR>(ID_CLEAR)), hInst, nullptr);
