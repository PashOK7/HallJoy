...
{
    mode = std::clamp(mode, (UINT)SettingsSparkPollMode_Safe, (UINT)SettingsSparkPollMode_MaxBurst);
    const UINT previous = g_sparkPollMode.load(std::memory_order_acquire);
    g_sparkPollMode.store(mode, std::memory_order_release);
    if (previous != mode)
        StabilityTrace_Write(L"INFO", L"spark.settings", L"poll_mode", L"old=%u new=%u", previous, mode);
}

UINT Settings_GetSparkPollMode()
{
    UINT mode = g_sparkPollMode.load(std::memory_order_acquire);
    return std::clamp(mode, (UINT)SettingsSparkPollMode_Safe, (UINT)SettingsSparkPollMode_MaxBurst);
}

void Settings_SetSparkRowLimit(UINT rows)
{
    rows = std::clamp<UINT>(rows, 0u, 8u);
    const UINT previous = g_sparkRowLimit.load(std::memory_order_acquire);
    g_sparkRowLimit.store(rows, std::memory_order_release);
    if (previous != rows)
        StabilityTrace_Write(L"INFO", L"spark.settings", L"row_limit", L"old=%u new=%u", previous, rows);
}

UINT Settings_GetSparkRowLimit()
{
    return std::clamp<UINT>(g_sparkRowLimit.load(std::memory_order_acquire), 0u, 8u);
}
