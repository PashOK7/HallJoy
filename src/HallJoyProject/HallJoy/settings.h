...
{
    SettingsSparkPollMode_Safe = 0,       // sleep after each successful route row
    SettingsSparkPollMode_FastYield = 1,  // yield after each successful route row
    SettingsSparkPollMode_MaxBurst = 2,   // keep polling, only yield every few rows
};
void Settings_SetSparkPollMode(UINT mode);
UINT Settings_GetSparkPollMode();
void Settings_SetSparkRowLimit(UINT rows); // 0 = auto/discovered rows, 1..8 = experimental cap
