...
std::vector<Candidate> Enumerate(bool log) {
  ...
  for (DWORD i = 0;; ++i) {
    ...
    if (log)
      DebugLog_Write(
          L"[chilkey.slice75.enumerate] path_hash=%016llX version=%04X "
          L"exact=%d",
...
bool InstallMap(const std::array<std::uint16_t, mg::kSlots> &assigned) {
  std::array<halljoy::native_layout::Key, 80> keys{};
  std::size_t count = 0;
  ...
  const auto token =
      halljoy::layout_identity::Token("chilkey-slice75", "SLICE75-1CA3-0701");
  if (count != 80 ||
...
bool Run(const Candidate &c) {
  ...
  if (!ready) {
    ++g_bad;
    DebugLog_Write(L"[chilkey.slice75] admission failed phase=%hs error=%lu",
                   phase, GetLastError());
    Clear();
    return false;
  }
  if (!NativeAnalogRouting_Claim(mg::kVendorId, mg::kProductId, c.path.c_str(),
                                 NativeAnalogProtocol::ChilkeySlice75) &&
  ...
  DebugLog_Write(L"[chilkey.slice75] connected path_hash=%016llX mapped=80 "
                 L"range_um=3300 experimental=1 reads=2B,23,12 calibration=0",
  ...
  while (!g_stop.load()) {
    ...
    if (now >= logAt) {
      DebugLog_Write(
          L"[chilkey.slice75] health half_updates=%llu failures=%llu "
          L"avg_us=%u max_us=%u",
  ...
  DebugLog_Write(
      L"[chilkey.slice75] session ended stop=%d error=%lu failures=%llu",
...
unsigned __stdcall Worker(void *) {
  try {
      ...
      for (const auto &c : candidates) {
        ...
        if (NativeAnalogRouting_IsClaimed(c.path.c_str()) &&
            !NativeAnalogRouting_IsClaimedBy(
                c.path.c_str(), NativeAnalogProtocol::ChilkeySlice75))
          continue;
        Run(c);
        if (g_stop.load())
          break;
    ...
    ++g_bad;
    DebugLog_Write(L"[chilkey.slice75] worker exception; input cleared");
  }
  Clear();
  ...
  *out = {};
  ...
  out->lastUpdateAgeMs =
      last && now >= last ? static_cast<std::uint32_t>(std::min<std::uint64_t>(
                          ...
                          : 0;
  _snwprintf_s(
      out->status, _countof(out->status), _TRUNCATE,
      L"Slice75 HE experimental: independent 6x21 travel, fixed 3.3 mm "
      L"range; hardware testing pending");
}
} // namespace
const NativeAnalogBackendDescriptor &Slice75_GetNativeBackendDescriptor() {
  static const NativeAnalogBackendDescriptor d{
      kNativeAnalogBackendAbiVersion,
      sizeof(NativeAnalogBackendDescriptor),
      "chilkey-slice75",
      L"Chilkey Slice75 HE (experimental)",
      NativeAnalogProtocol::ChilkeySlice75,
