...
checks = {
    ...
    'native protocols preserve ordinary-HID multi-device analogue max while extended native keys avoid duplicate SDK reads': ('NativeAnalogBackends_ReadMilli(hidKeycode)' in backend and 'Arbitrate({ hidKeycode' in backend and '(!native.owned || halljoy::keycode::IsStandardHid(hidKeycode))' in backend),
    'unified output scheduler is wired': all(x in backend for x in ('VigemOutputScheduler::Decision::DeferUntilDeadline', 'The scheduler remains in realtime', 'Backend_GetNextOutputDeadlineQpc')),
    ...
    'duplicate ViGEm keepalive is removed': ('if (!changed)' in backend and 'kKeepAliveUs' not in backend),
