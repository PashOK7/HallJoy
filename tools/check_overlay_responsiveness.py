...
def main() -> int:
    ...
    with connect_when_ready(address, args.connect_deadline_ms) as bootstrap:
        bootstrap.settimeout(1.0)
        send_get(bootstrap, args.port, "/", "close")
        status, headers, _ = read_response(bootstrap)
        if " 200 " not in status:
            raise SystemExit(f"FAIL: overlay bootstrap returned {status}")
        cookie = headers.get("set-cookie", "").split(";", 1)[0]
        if not cookie.startswith("HallJoySession=") or len(cookie) != 47:
            raise SystemExit("FAIL: overlay bootstrap did not issue a 128-bit session cookie")

    with socket.create_connection(address, timeout=1.0) as telemetry:
        telemetry.settimeout(1.0)
        send_get(telemetry, args.port, "/client_perf?frames=1", "keep-alive", cookie, origin)
    ...
    with socket.create_connection(address, timeout=1.0) as state:
        ...
        send_get(state, args.port, "/state", "close", cookie, origin)
        status, headers, body = read_response(state)
    ...
    if headers.get("access-control-allow-origin") != origin:
        raise SystemExit("FAIL: state response did not echo the exact allowed origin")
